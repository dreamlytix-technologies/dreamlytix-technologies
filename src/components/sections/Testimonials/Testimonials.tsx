...
const Stars = ({ count = 5 }: { count?: number }) => (
  <div className="flex gap-0.5" role="img" aria-label={`${count} out of 5 stars`}>
    {[...Array(5)].map((_, i) => (
      <Star key={i} className={`w-4 h-4 ${i < count ? "text-[#FBBC05] fill-[#FBBC05]" : "text-gray-300"}`} />
    ))}
  </div>
);
...
const reviews = [
  ...
  },
  {
    ...
    text: "The school management system they built transformed how we operate. Attendance, fees, communication — all in one place. Parents love the mobile app. Truly a game-changer!",
    rating: 5,
    initials: "PS",
    color: "#34A853",
    time: "1 month ago",
  ...
  },
  {
    ...
    text: "The admin panel for our school is exceptional. Teachers, parents, and staff all find it very easy to use. The Dreamlytix team was patient and thorough with training too.",
    rating: 5,
    initials: "DR",
    color: "#4285F4",
    time: "5 weeks ago",
    ...
    <div className="flex-shrink-0 w-72 md:w-80 bg-white rounded-2xl shadow-lg shadow-black/10 p-5 border border-gray-100 hover:shadow-xl hover:shadow-blue-100/40 hover:-translate-y-1 transition-all duration-300 cursor-default">
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0" style={{ background: review.color }}>
            {review.initials}
          </div>
          <div>
      ...
      {/* Review text */}
      <p className="text-gray-600 text-sm leading-relaxed line-clamp-4">&ldquo;{review.text}&rdquo;</p>
...
export default function Testimonials() {
  ...
  const row2 = useMemo(() => [...reviews.slice().reverse(), ...reviews.slice().reverse()], []);

  return (
    <section id="testimonials" className="relative py-20 overflow-hidden" style={{ background: "linear-gradient(135deg, #0b1a38 0%, #0d2150 50%, #0b1a38 100%)" }}>
      {/* Dot-grid overlay */}
      <div
      ...
      />
      {/* Glows */}
      <div className="absolute top-0 left-1/4 w-96 h-96 rounded-full blur-[120px] pointer-events-none" style={{ background: "rgba(66,133,244,0.12)" }} />
      <div className="absolute bottom-0 right-1/4 w-80 h-80 rounded-full blur-[100px] pointer-events-none" style={{ background: "rgba(52,168,83,0.10)" }} />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-12">
          <m.div
            ...
            viewport={{ once: true }}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 border border-white/20 text-white text-xs font-semibold backdrop-blur-sm mb-5"
          >
            <GoogleIcon size={14} />
            Google Reviews
          </m.div>
          ...
          >
            What Our{" "}
            <span className="text-transparent bg-clip-text" style={{ backgroundImage: "linear-gradient(90deg,#4285F4,#34A853,#FBBC05,#EA4335)" }}>
              Clients Say
            </span>
          </m.h2>
          ...
          <m.p
            ...
            className="text-blue-200 text-sm md:text-base max-w-3xl mx-auto"
          >
            Real reviews from schools, gyms, and businesses who trust Dreamlytix to power their digital journey.
          </m.p>
        </div>
        ...
        >
          ...
          <div className="bg-white rounded-2xl px-8 py-5 shadow-2xl flex items-center gap-6">
            <div className="text-center">
              ...
              <div className="text-5xl font-black text-gray-900 leading-none">5.0</div>
              <Stars count={5} />
              <p className="text-gray-400 text-xs mt-1">Based on {reviews.length}+ reviews</p>
            </div>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-2 bg-white/10 border border-white/20 rounded-full px-4 py-2 text-white text-sm backdrop-blur-sm">
              <svg className="w-4 h-4 text-[#34A853]" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z" />
              </svg>
              All reviews verified by Google
            </div>
            <div className="flex items-center gap-2 bg-white/10 border border-white/20 rounded-full px-4 py-2 text-white text-sm backdrop-blur-sm">
              <svg className="w-4 h-4 text-[#FBBC05]" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <div className="relative mb-5">
          {/* Fade edges */}
          <div className="absolute left-0 top-0 bottom-0 w-24 z-10 pointer-events-none" style={{ background: "linear-gradient(to right, #0b1a38, transparent)" }} />
          <div className="absolute right-0 top-0 bottom-0 w-24 z-10 pointer-events-none" style={{ background: "linear-gradient(to left, #0b1a38, transparent)" }} />
        ...
        {/* ── CTA ── */}
        <m.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="text-center mt-12">
          <a
