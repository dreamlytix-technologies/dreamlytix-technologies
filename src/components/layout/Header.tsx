...
const Header = () => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "py-3 glass-dark shadow-lg" : "py-6 bg-transparent"}`}
    >
        ...
        <nav className="flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center group">
            ...
            <div className="flex flex-col leading-tight">
              ...
              <span className="text-[10px] sm:text-xs text-center text-gray-500 tracking-wide">
                You Dream, We Build.
              </span>
            </div>
          </Link>
          ...
          {/* Desktop Navigation */}
          <ul className="hidden md:flex items-center space-x-8 text-sm lg:text-base font-normal">
            {navLinks.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
                  className="relative text-gray hover:text-orange-500 transition-colors group"
                >
                  {link.label}
